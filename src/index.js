...
if (number === 0) { //проверка на ноль
  return 'zero'
} else { //если не ноль то запускаем функцию
  toText(number)
}
  function toText(number) {
    if (values.find(item => item.value === number)) { //если такое число встречается то записываем его
  text = values.find(item => item.value === number).str;
  if (result.length === 0) {//проверка для пробела
    result+= text;
  } else {
    result+= ' ' + text;
  }
